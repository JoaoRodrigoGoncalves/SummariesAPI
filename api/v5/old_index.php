                ...
                switch($_SERVER['REQUEST_METHOD']){
                    case "GET":
                        if(isset($_SERVER['HTTP_X_API_KEY']))
                        {
                            if(isset($uri[$opertation+1])){
                                if(is_numeric($uri[$opertation+1])){
                                    if(isset($uri[$opertation+2])){
                                        if(is_numeric($uri[$opertation+2])){
                                            /**
                                             * /user/<userID>/<workspaceID>
                                             *  INVALID REQUEST
                                             */
                                            if(isset($uri[$opertation+3])){
                                                if($uri[$opertation+3] === "summary"){
                                                    if(isset($uri[$opertation+4])){
                                                        if(is_numeric($uri[$opertation+4])){
                                                            if(isset($uri[$opertation+5])){
                                                                if($uri[$opertation+5] === "files"){
                                                                    if(isset($uri[$opertation+6])){
                                                                        /**
                                                                         * /user/<userID>/<workspaceID>/summary/<summaryID>/files/<file>
                                                                         */

                                                                    }else{
                                                                        /**
                                                                         * /user/<userID>/<workspaceID>/summary/<summaryID>/files
                                                                         */
                                                                ...
                                                                /**
                                                                 * /user/<userID>/<workspaceID>/summary/<summaryID>
                                                                 */
                                                        ...
                                                        /**
                                                         * /user/<userID>/<workspaceID>/summary
                                                         */
                    ...
                    case "PUT":
                        if(!is_null($uri[$opertation+1]) || !empty($uri[$opertation+1])){
                            // https://lornajane.net/posts/2008/accessing-incoming-put-data-from-php
                            $PUT_vars = parse_str(file_get_contents("php://input"), $PUT_vars);
                            if(is_numeric($uri[$opertation+1]) && isset($uri[$opertation+2])){
                                if($uri[$opertation+2] === "changepassword"){
                                    if(isset($_SERVER['HTTP_X_API_KEY'])){
                                        $AccessToken = mysqli_real_escape_string($connection, $_SERVER['HTTP_X_API_KEY']);
                                        $isValid = $authTokens->isTokenValid($AccessToken);
                                        $userID = mysqli_real_escape_string($connection, $uri[$opertation+1]);
                                        ...
                                        if($isValid){
                                            if($userID==$isValid || $userFunctions->isUserAdmin($isValid)){
                                                $userPassword = (isset($PUT_vars["password"]) ? $PUT_vars["password"] : $settings->defaultPassword);
                                                if($userFunctions->UpdateUserPassword($userID, $userPassword)){
                                                    header("HTTP/1.0 200 OK");
                                                    $response['status'] = true;
                                                    $response['errors'] = "";
                                                }else{
                                                    header("HTTP/1.0 500 Internal Server Error");
                                                    $response['status'] = false;
                                                    $response['errors'] = "An Error Occurred While Trying to Update the User.";
                                                }
                                            }else{
                                                header("HTTP/1.0 403 Forbidden");
                                                $response['status'] = false;
                                                $response['errors'] = "Destination user is not administrator or does not match authentication";
                                            }
                                        }else{
                                            header("HTTP/1.0 401 Unauthorized");
                                            $response['status'] = false;
                                    ...
                                    header("HTTP/1.0 404 Not Found");
                                    $response["status"] = false;
                                    $response["errors"] = "Operation Not Found";
                                }
                            }else if(is_numeric($uri[$opertation+1])){
